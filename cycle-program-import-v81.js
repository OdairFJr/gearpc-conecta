(() => {
  if (window.__GEARPC_CYCLE_IMPORT_V81__) return;
          ...
          <div class="detail-shell">
            <div class="dialog-title-row sticky-dialog-header">
              <div>
                ...
                <h2>Converter arquivo para o Conecta</h2>
                <p class="dialog-helper">Envie o ciclo já montado no computador. O app interpreta o arquivo e monta uma versão para conferência antes de salvar.</p>
              </div>
            ...
            <section id="cycleImportPickV81" class="cycle-import-v81-box">
              <label class="cycle-import-v81-file">
                <strong>Arquivo do ciclo</strong>
                <input id="cycleImportFileV81" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />
                <small>PDF, Word (.docx) ou Excel (.xlsx/.xls). Arquivos .doc antigos precisam ser salvos como .docx.</small>
              </label>
              <button id="cycleImportReadV81" type="button" class="save-button">Interpretar arquivo</button>
            </section>
            ...
            <form id="cycleImportReviewV81" class="hidden">
              ...
              <div class="dialog-actions sticky-dialog-actions">
                <button id="cycleImportBackV81" type="button" class="cancel-button">Escolher outro arquivo</button>
  ...
  async function interpretFile() {
    const file = $('cycleImportFileV81')?.files?.[0];
    if (!file) { setMsg('Selecione um arquivo para importar.'); return; }
    const button = $('cycleImportReadV81');
    button.disabled = true;
    button.textContent = 'Lendo arquivo...';
    setMsg('Extraindo o conteúdo do arquivo...');
    try {
      const text = await extractText(file);
      if (!String(text).trim() || String(text).trim().length < 30) {
        throw new Error('Não encontrei texto suficiente no arquivo. Se for um PDF escaneado como imagem, esta versão ainda não consegue interpretá-lo.');
      }
      setMsg('Interpretando o ciclo e organizando as previsões...');
      const { data, error } = await rt.client.functions.invoke('importar-ciclo-programa-ia', {
        body: { filename:file.name, text:String(text).slice(0, 90000) }
      });
      if (error) throw error;
      if (!data?.cycle) throw new Error('O arquivo foi lido, mas não foi possível montar o ciclo.');
      imported = data.cycle;
      fillReview(imported);
    } catch (error) {
      console.error(error);
      setMsg(error?.message || 'Não foi possível importar este arquivo.');
    } finally {
      button.disabled = false;
      button.textContent = 'Interpretar arquivo';
    }
  }
  ...
  async function saveImported(event) {
    ...
    try {
      const { data:cycle, error } = await rt.client.from('ciclos_programa').insert({
        ramo_id:ramoId, nome, data_inicio:start, data_fim:end, objetivo,
